        ...
        <aside className="w-64 bg-slate-900/80 border-r border-slate-800 flex flex-col h-full flex-shrink-0">
            <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto mt-4">
                {menuItems.map(({ path, icon, label }) => (
                    <Link 
                        key={path} to={path} 
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors font-medium ${pathname === path ? 'bg-indigo-500/10 text-indigo-400' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}`}
                    >
            ...
            <div className="p-4 border-t border-slate-800/50">
                {status === 'checking' && (
                    <div className="flex items-center gap-2 text-slate-400 bg-slate-500/10 px-3 py-2 rounded-lg border border-slate-500/20 transition-all justify-center">
