    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
            ...
            <div className="w-full lg:w-72 flex-shrink-0 relative">
                ...
                <Card className="sticky top-8 border-slate-800/60 shadow-[0_8px_30px_rgb(0,0,0,0.4)] bg-gradient-to-b from-slate-900/95 to-slate-950/95 backdrop-blur-xl relative z-10">
                    <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-800">
                        <h2 className="text-lg font-extrabold text-white flex items-center gap-2.5 tracking-tight"><Filter size={18} className="text-indigo-400"/> Filters</h2>
                        {Object.values(filters).some(v => v !== '') && (
                            <button onClick={resetFilters} className="text-xs font-bold text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 px-3 py-1.5 rounded-md transition-all flex items-center gap-1.5 border border-slate-700 hover:border-slate-600">
                                <X size={12}/> Reset
                            </button>
                        )}
                    </div>
                    <div className="space-y-6">
                        <div>
                            ...
                            <div className="relative group">
                                <Search className="absolute left-3.5 top-3 text-slate-500 group-focus-within:text-indigo-400 transition-colors" size={16} />
                                <input 
                                    type="text" placeholder="Find by name..."
                                    className="w-full pl-10 pr-3 py-2.5 bg-slate-900/50 border border-slate-700/50 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner placeholder:text-slate-500"
                        ...
                        <div>
                            ...
                            <select className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl p-2.5 text-white text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner" value={filters.language} onChange={e => setFilters({...filters, language: e.target.value})}>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Use Case / Style</label>
                            <select className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl p-2.5 text-white text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all shadow-inner" value={filters.use_case} onChange={e => setFilters({...filters, use_case: e.target.value})}>
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <button 
                                    ...
                                    className={`flex-1 py-2.5 text-sm font-semibold rounded-xl border transition-all ${filters.gender === '' ? 'bg-indigo-600 border-indigo-500 text-white shadow-[0_0_15px_rgba(79,70,229,0.3)]' : 'bg-slate-900/50 border-slate-700/50 text-slate-400 hover:text-white hover:border-slate-600'}`}
                                >All</button>
                                <button 
                                    onClick={() => setFilters({...filters, gender: 'Male'})}
                                    className={`flex-1 py-2.5 text-sm font-semibold rounded-xl border transition-all ${filters.gender === 'Male' ? 'bg-indigo-600 border-indigo-500 text-white shadow-[0_0_15px_rgba(79,70,229,0.3)]' : 'bg-slate-900/50 border-slate-700/50 text-slate-400 hover:text-white hover:border-slate-600'}`}
                                >Male</button>
                                <button 
                                    onClick={() => setFilters({...filters, gender: 'Female'})}
                                    className={`flex-1 py-2.5 text-sm font-semibold rounded-xl border transition-all ${filters.gender === 'Female' ? 'bg-indigo-600 border-indigo-500 text-white shadow-[0_0_15px_rgba(79,70,229,0.3)]' : 'bg-slate-900/50 border-slate-700/50 text-slate-400 hover:text-white hover:border-slate-600'}`}
                                >Female</button>
                            </div>
                        </div>
                    </div>
            ...
            <div className="flex-1">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4 border-b border-slate-800/60 pb-6 relative">
                ...
                {loading ? (
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16 text-slate-400 bg-slate-900/30 rounded-xl border border-slate-800">
                    ...
                    <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                        {voices.map(voice => {
                            ...
                            <motion.div key={voice.id} variants={item} whileHover={{ y: -4 }}>
                                <Card className="flex flex-col h-full border border-slate-800/60 bg-gradient-to-b from-slate-900/90 to-slate-950/90 backdrop-blur-xl hover:border-indigo-500/40 hover:shadow-[0_8px_30px_rgb(99,102,241,0.15)] transition-all duration-300 group relative overflow-hidden">
                                        ...
                                        <div className="flex flex-col items-end gap-1.5">
                                            {voice.is_cloned ? (
                                                ...
                                                <div className="flex flex-col items-end gap-1">
                                                    <span className="text-[9px] font-bold uppercase tracking-wider bg-slate-800/80 text-slate-300 px-2 py-1 rounded-md border border-slate-700/80">{voice.engine === 'piper' ? 'Offline' : voice.engine === 'coqui' ? 'Coqui' : 'Online'}</span>
                                    ...
                                    <div className="relative z-10 flex flex-wrap gap-2 mb-5 mt-2">
                                        ...
                                        {toneTags.slice(0, 2).map((tone, i) => (
                                            <span key={i} className="text-[11px] font-medium text-slate-300 bg-slate-800/60 border border-slate-700/60 px-2.5 py-1 rounded-full">
                                                {tone}
                                            </span>
                                        ))}
                                    </div>
                                    
                                    <div className="relative z-10 flex items-center gap-3 mt-auto pt-5 border-t border-slate-800/60">
                                        <motion.button 
                                            whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                            onClick={() => handlePlayPreview(voice)}
                                            disabled={loadingPreviewId === voice.id}
                                            className={`w-11 h-11 rounded-full flex items-center justify-center text-white transition-all shadow-lg ${playingId === voice.id ? 'bg-indigo-600 shadow-[0_0_20px_rgba(79,70,229,0.5)] ring-2 ring-indigo-400 ring-offset-2 ring-offset-slate-950' : 'bg-slate-800 hover:bg-indigo-600 hover:shadow-[0_0_15px_rgba(79,70,229,0.4)] border border-slate-700 hover:border-indigo-500'}`}
                                        >
                                        ...
                                        <Button onClick={() => handleUseInStudio(voice.id)} variant="secondary" className="flex-1 py-2.5 text-sm font-semibold border border-slate-700 hover:border-slate-500 hover:bg-slate-800 bg-slate-800/50 shadow-inner hover:shadow-none transition-all">
                                            Use in Studio
                                        </Button>
                                        {voice.is_cloned && (
                                            <button 
                                                onClick={() => handleDeleteVoice(voice.id, voice.name)}
                                                className="w-11 h-11 rounded-xl flex items-center justify-center text-slate-400 hover:text-red-400 hover:bg-red-500/10 border border-transparent hover:border-red-500/30 transition-all shadow-sm"
